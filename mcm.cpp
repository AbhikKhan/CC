...
/**************************************************************************************/
ll helper(vt<ll> &pins, int i, int j){
	ll n = pins.size();
	if(i >= j)return 0;
	if(i + 1 == j)return pins[i] * pins[j] * (i - 1 < 0? 1: pins[i - 1]) * (j + 1 >= n? 1: pins[j + 1]);
	// loop from i = start to end
	ll max_cost = 0;
	for(int k = i; k< j; k+=2){
		ll cost = ((i - 1) < 0? 1: pins[i - 1]);
	        ll tC = 0;
		cost *= ((j + 1 >= n)?1:pins[j + 1]);
		cost *= pins[k];
		for(int l = 1; l + k <= j; l+=2){
			ll temp = cost;
			temp *= pins[k + l];
			temp += helper(pins, i, k - 1) + helper(pins, k + 1, k + l - 1) + helper(pins, k + l + 1, j);
			tC = max(tC, temp);
		}	
		max_cost = max(max_cost, tC);	
	}
	// return the max cost
	return max_cost;

}
ll getCost(vt<ll> &pins){
	int n = pins.size();
	return helper(pins, 0, n-1);
}
